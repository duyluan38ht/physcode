...
class SayWhatProListTable extends WPListTableExportable implements SayWhatProListTableInterface {
	...
	function __construct( SayWhatProSettingsInterface $settings ) {
		...
		parent::__construct();
		$this->export_button_text = __( 'Export replacements', 'say_what' );
	}
	...
	function get_columns() {
		$columns = array(
			...
			'string_id'          => _x( 'ID', 'Unique ID of the string replacement', 'say_what' ),
	 ...
	 */
	function column_orig_string( $item ) {
	 ...
	 * @return string       The output for the column.
	 */
	function column_csv_orig_string( $item ) {
