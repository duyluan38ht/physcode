...
class PBLW_Plugin_Updater {
	...
	public function save_licence() {
		...
		if ( $is_valid ) {
			...
			echo "<div id='message' class='error'><p>Sorry, but your licence cannot be validated. Either the licence is invalid, or is already in use on another site. If you believe this is an error please <a target='_blank' href='https://plugins.leewillis.co.uk/support/''>contact support</a>.</p></div>";
		}
	}
	...
	public function validate_licence( $licence ) {
		...
		if ( true == $licence_data->success && 'valid' == $licence_data->license ) {
			update_option( 'pblw_licence_status_' . $this->slug , $licence_data->license );
			return true;
		}
		return false;
	}
	...
	public function licence_admin_page() {
		...
		<form method="post">
			...
			<p><label for="licence_code">Licence Code: </label><input type="text" size="40" name="licence_code" placeholder="Enter your licence code" value="<?php echo esc_attr( $licence_code ); ?>"></p>
